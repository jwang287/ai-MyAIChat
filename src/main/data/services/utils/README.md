...
```ts
function mergeBuiltinWithOptionalRow(def: BuiltinDefinition, dbRow?: DbRow): BuiltinDefinition {
  return {
    /* ... builtin fields ... */
...
| Merge path where the source row itself may be absent (e.g. builtin + optional row) | `timestampToISOOrUndefined(dbRow?.createdAt)` |
...
Backs provider logo slots and historic Mini App logo references. A *single-file slot* is an association table where one owner row holds at most one file: the ref row is the single source of truth for that owner's uploaded file, and the owner row keeps only a preset key.
