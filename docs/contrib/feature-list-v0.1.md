...
| 非核心功能与扩展 | 14 | 5 项保留、9 项移除 | 已确认保留的扩展能力与不纳入精简版的能力。 |
...
API 网关作为已确认扩展能力保留。应默认仅监听本机地址，并继续维护访问鉴权、端口冲突、
网络暴露提示和滥用防护；需要局域网访问时必须由用户明确启用。
...
- API 网关默认仅允许本机访问；局域网访问必须经过用户明确启用，并受鉴权与网络暴露提示保护。
