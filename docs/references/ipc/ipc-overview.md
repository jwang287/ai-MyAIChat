...
1. **Source trust** (`validateSender`): one channel funnels every capability, so verify the caller first. Embedded web content is never trusted: `<webview>` guests are rejected by WebContents type, and only the **top-level frame** may be trusted. Its URL must be the app's own — in production a `file:` path **inside the app bundle root** (`application.getPath('app.root')`), so any other local file (a downloaded/exported HTML opened in an `ipcRenderer`-reachable window) is rejected; in development, exactly the dev-server origin. Remote origins are rejected.
